...
export function useMyPositions() {
  ...
  const formattedPositions =
    positionIds
      ?.map((id, index) => {
        const detailItem = detailsData?.[index] as
          | ContractCallResult
          | undefined;
        const pnlItem = detailsData?.[index + positionIds.length] as
          | ContractCallResult
          | undefined;
        ...
        try {
          countryName = hexToString(detailResult.countryCode, {
            size: 32,
          }).replace(/\0/g, "");
        } catch (e) {
        ...
        return {
          ...
          entryTime: new Date(
            Number(detailResult.entryTimestamp) * 1000
          ).toLocaleString(),
        };
      })
      .filter(Boolean) || [];
